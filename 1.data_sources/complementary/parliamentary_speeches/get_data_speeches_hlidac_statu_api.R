...
for (i in seq_len(pages)[seq_len(pages) <= 200]) {
  ...
  hlidac_data[[i]] <- fromJSON(content(httr::GET(
    url = "https://www.hlidacstatu.cz/api/v2/datasety/stenozaznamy-psp/hledat",
    httr::add_headers(
      accept = "application/json",
      Authorization = token
...
get_all_speeches(dir_name = "1.data_sources/complementary/parliamentary_speeches/data", # Specify the folder, where the tables will be saved
