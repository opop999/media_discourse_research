...
```{r include=FALSE}
  # Package names
  packages <- c("dplyr", "stringr", "purrr", "readr", "jsonlite")
...
date_seq <- seq(as.Date("2015-01-01T00:00:00"), as.Date("2022-05-24T23:59:59"), by = "4 days")
...
as.Date(setdiff(as.character(seq(as.Date("2015-01-01T00:00:00"),
                                 as.Date("2022-05-24T23:59:59"), by = "1 day")),
...
date_seq_period <- seq(as.Date("2015-01-01"), as.Date("2022-05-01"), by = "1 months")
...
date_seq <- seq(as.Date("2015-01-01"), as.Date("2022-05-25"), by = "3 days")
...
full_articles_df_cz_sk <- list.files(path = "media_articles/data/full/chunks_cz_sk/", pattern = "*.rds", full.names = TRUE) %>%
  ...
  filter(PublishDate >= as.Date("2022-02-01")) # We can filter by specific date, if we are adding new data

# Read the annotated dataset of chunks back in
annotated_df <- list.files(path = "media_articles/data/annotations/chunks/", pattern = "*.rds", full.names = TRUE) %>%
  map_dfr(readRDS) %>% 
  mutate(datePublished = as.Date(datePublished)) %>% 
  filter(datePublished >= as.Date("2022-02-01"))
...
full_articles_df_cz_sk <- list.files(path = "media_articles/data/full/chunks_cz_sk/", pattern = "*.rds", full.names = TRUE) %>%
  map_dfr(readRDS) %>% 
  mutate(PublishDate = as.Date(PublishDate)) %>% 
  filter(PublishDate >= as.Date("2022-02-01")) 

# Read the additional articles dataset back in
additional_full_articles_df <- readRDS("media_articles/data/full/additional_full_articles_df.rds")
...
saveRDS(combined_newton_media_types, "media_articles/data/media_type_labels/combined_newton_media_types.rds")
...
Step 1: Clean the dataset from Cvrcek et al., keeping only fully matching labels between coders
```{r}
# There are two human coders who assign labels to the media
process_media_labels_dataset <- function(rds_path) {
processed_df <-
  readRDS(rds_path) %>%
  transmute("name" = "web", "name_alt" = "title", "media_type", "media_type_monitora" = "Monitora kategorie") %>% 
  set_names(c("name", "name_alt", "media_type", "media_type_monitora")) %>%
...
labels_df_1 <- process_media_labels_dataset(paste0("media_articles/data/media_type_labels/labels_vc_", last_update, ".csv"))
labels_df_2 <- process_media_labels_dataset(paste0("media_articles/data/media_type_labels/labels_jh_", last_update, ".csv"))
...
Step 2: Add our custom labels for "offline" outlets and other important ones which are not present in the Vaclav Cvrcek et al. dataset
```{r}
