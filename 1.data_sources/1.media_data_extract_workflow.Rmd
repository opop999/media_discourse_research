...
```{r}
unique_media_and_types <- annotated_df[,c("sourceName", "mediaType.id")] %>%
  distinct() %>% 
  transmute(name = sourceName,
         media_type_id = as.character(mediaType.id)) %>% 
  # Dataset has erroneusly "Stream.cz" as a TV and Online. We delete the TV record.
  filter(name != "Stream.CZ" | media_type_id != "4")

saveRDS(unique_media_and_types, "media_articles/data/media_type_labels/unique_media_types.rds")
```
...
unique_media_and_types <- readRDS("media_articles/data/media_type_labels/unique_media_types.rds")
...
if (file.exists(media_names_and_ids_clean_path)) {
  unique_media_and_types <- unique_media_and_types %>%
    filter(!name %in% readRDS(media_names_and_ids_clean_path)[["searched_name"]])
}
...
history_id_df_raw_list <- get_history_id(
  media_name_vec = unique_media_and_types$name,
  media_types_vec = as.numeric(unique_media_and_types$media_type_id),
  page_size = 3,
  newton_api_token = Sys.getenv("NEWTON_TOKEN"),
  log = TRUE,
  log_path = "media_articles/docs/"
...
saveRDS(media_count_df_by_period, "media_articles/data/counts/media_count_df_by_1_month.rds")
...
```{r}
full_articles_cz %>% transmute(name = SourceName) %>% count(media_name, sort = TRUE) %>% saveRDS("media_articles/data/counts/newton_migration_articles_by_media.rds")
```
...
saveRDS(combined_newton_media_types, "media_articles/data/media_type_labels/website_newton_media_types.rds")
...
# Alternative, preserving the unlabeled media and adding new labels from Newton
...
Alternative workflow: getting media labels from Korpus.cz "Online 2 Now" corpus, published in late 2022

```{r}
# Path to .csv files, each contains names of media outlets (doc.resource) 
# under the given media category (doc.media_type).
# These data were extracted from the KonText tool using Frequency summaries. 
# Their names reflect the given media type, such as antisystemic.csv, investigative.csv, etc.

path_to_korpus_files <- "media_articles/data/media_type_labels/korpus_cz"

new_labels <- list.files(path_to_korpus_files, full.names = TRUE)
# Use the name of the .csv file to determine the media type, while removing the last four characters (file extension)
names(new_labels) <- list.files(path_to_korpus_files, full.names = FALSE) %>% substr(1, nchar(.) - 4)

read_articles <- function(file_path) {
  file_path %>% 
  read_csv2(col_names = c("rank", "media_name", "freq", "i.p.m"), col_types = "ncnn") %>%
    transmute(media_name, type = names(file_path))
}

korpus_labels <- lapply(new_labels, read_articles) %>% 
  bind_rows(.id = "media")

saveRDS(korpus_labels, "media_articles/data/media_type_labels/korpus_labels.rds")

```

```{r}
# Remove duplicates where the same media is in more than one category
korpus_labels <- readRDS("media_articles/data/media_type_labels/korpus_labels.rds") %>% 
  filter(!name %in% c(.$name[duplicated(.$name)], "ceskezpravy.eu", "raptor-tv.cz", "vipnoviny.cz"))

# Load custom dataset with labels.
# Work in progress: All above 100 articles in the whole period labelled
custom_media_labels <- read_csv("media_articles/data/media_type_labels/custom_media_labels.csv")

joined_labels <- bind_rows(korpus_labels, custom_media_labels) %>% distinct()
...
# Join our media labels dataset with the dataset of all document ids, joining by media_id key
all_media_labels_with_doc_id  <- inner_join(joined_labels, full_articles_cz, by = c("name" = "media_name")) %>% distinct()
