...
ner_labels <- list(
  ...
  ner_alternative_1 %>%
    ...
    ylab(element_blank()) +
    xlab("Occurences per million words") +
    labs(title = "Top 30 Named Entities in Czech Alternative Media, January 2015-December 2015") +
    theme_classic() +
    theme(plot.title = element_text(face = "bold", size = 8),
          axis.title.y = element_text(size = 8),
          plot.margin = margin(7,30,3,5, "pt")) +
    ...
    scale_y_discrete(
    labels = rev(ner_labels$czech)
  ) +
    geom_text(aes(label = ner_labels$english, x = 10, y = ent_text_stemmed, fontface=2), color = "white", size = 2.5, hjust = 0)

ggsave(filename = "ner_minimalist_labels.png", device = "png", width = 3075, height = 1250, units = "px")
```
...
```{r}
# Copy the top entities to clipboard for easier translation
period_data %>%
    transmute(measure = n_ipm,
              entity = reorder(ent_text_stemmed, n_ipm)) %>% 
    slice_max(measure, n = 15) %>%
  pull(entity) %>% 
  stringr::str_to_title() %>% 
  clipr::write_clip(object_type = "character", breaks = ",")
```
...
labels <- list(
  mainstream_1 = list(
    "czech" = c(
      "Evropa", "Německo", "Česko", "EU", "Sýrie", "Evropská Unie", "Česká Republika", "Praha", "Maďarsko", "Řecko", "Rusko", "Turecko", "Islámský Stát", "ČSSD", "Francie"
    ),
    "english" = c(
     "Europe", "Germany", "Czechia", "EU", "Syria", "European Union", "Czech Republic", "Prague", "Hungary", "Greece", "Russia", "Turkey", "Islamic State", "ČSSD", "France"
    )
  ),
  mainstream_3 = list(
    "czech" = c(
   "Ukrajina","Rusko","Česko","Praha","Česká Republika","Evropa","Kyjev","Evropská Unie","Německo","Polsko","Slovensko","ANO","EU","ODS","ČR"
    ),
    "english" = c(
    "Ukraine", "Russia", "Czechia", "Prague", "Czech Republic", "Europe", "Kyiv", "Europ. Uni.", "Germany", "Poland", "Slovakia", "ANO", "EU", "ODS", "CZ"
    )
  ),
  alternative_1 = list(
    "czech" = c(
      "Evropa", "EU", "Rusko", "Německo", "Sýrie", "USA", "Česká Republika", "Ukrajina", "Evropská Unie", "ČR", "Islámský Stát", "Řecko", "Turecko", "Západ", "Francie"
    ),
    "english" = c(
      "Europe", "EU", "Russia", "Germany", "Syria", "USA", "Czech Republic", "Ukraine", "European Union", "CZ", "Islamic State", "Greece", "Turkey", "West", "France"
    )
  ),
  alternative_3 = list(
    "czech" = c(
      "Ukrajina", "Rusko", "EU", "Evropa", "USA", "Německo", "NATO", "Česká Republika", "Západ", "ČR", "Česko", "Evropská Unie", "Čína", "Praha", "Polsko"
    ),
    "english" = c(
      "Ukraine", "Russia", "EU", "Europe", "USA", "Germany", "NATO", "Czech Republic", "West", "CZ", "Czechia", "Europ. Uni.", "China", "Prague", "Poland"
    )
  )
)
```
...
```{r}
period_data <- readRDS(paste0("data/ner_", period_type, ".rds")) %>% 
  transmute(measure = n_ipm,
         entity = reorder(ent_text_stemmed, n_ipm)) %>% 
  slice_max(measure, n = 15) %>% 
...
period_data %>% 
  ggplot(aes(x = measure, y = entity, label = round(measure, digits = 1))) +
    geom_col(fill = graph_color[[media_type]]) +
    geom_label(size = 1.7, label.padding = unit(0.09, "lines")) +
    theme_classic() +
  labs(
    y = element_blank(),
    x = "Instances per million",
    title = paste(
      "Top 15 Named Entities",
      "in Czech",
      stringr::str_to_title(media_type),
      "Media,",
      periods[[period_nr]]
    ),
    ...
    scale_x_continuous(
  expand = c(0, 0),
  limits = c(0, max(period_data$measure) + (max(period_data$measure)/15)),
  ...
  geom_text(aes(label = labels[[period_type]][["english"]], x = 10, y = entity, fontface=2), color = "white", size = 1.5, hjust = 0) +
...
ggsave(paste0("../../5.write_up/conference_bratislava_october_2023/graphics/ner_", period_type, ".png"), device = "png",
       width = 1920, height = 1080, units = "px")
```
