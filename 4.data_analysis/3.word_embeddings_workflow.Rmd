...
for (o in seq_along(periods_name)) {
  ...
  for (i in media_types) {
    period_dataset %>%  
      ...
      pull(text) %>% 
      paste(collapse = "") %>% 
      writeLines(paste0("word_embedding/data/corpora/", i, "_", periods_name[o],".txt"))
...
corpora_list <- list.files(path = "word_embedding/data/corpora",
                           pattern = "*.txt",
                           full.names = FALSE)
...
lapply(corpora_list, function(element) {
  system(
    paste0(
      "word_embedding/data/wang2vec_libs/word2vec -train word_embedding/data/corpora/",
      element,
...
for (i in models) {
  set.seed(3859)
  model <- read.word2vec(file = paste0("word_embedding/data/models/", i), normalize = TRUE)
