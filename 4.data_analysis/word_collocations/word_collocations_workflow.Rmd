...
```{r}
stats_alternative_3[["migrant"]] %>%
  tibble::enframe("collocate", "measure") %>%
  mutate(collocate = fct_reorder(collocate, measure)) %>%
  slice_max(order_by = measure, n = 15) %>%
  bind_cols("original_label" = labels[["alternative_3"]][["czech"]],
            "translated" = labels[["alternative_3"]][["english"]]) %>%
  ggplot(aes(x = measure, y = collocate, label = round(measure, digits = 1))) +
    geom_col() +
    geom_label(size = 1.7, label.padding = unit(0.09, "lines")) +
    theme_classic() +
  ...
  limits = c(0, 15)
    ...
    scale_y_discrete(
    labels = rev(labels[["alternative_3"]][["czech"]])
  ) +
  geom_text(aes(label = labels[["alternative_3"]][["english"]], x = 0.1, y = collocate, fontface=1), color = "white", size = 2.5, hjust = 0) +
...
ggsave("../../5.write_up/conference_bratislava_october_2023/graphics/collocs_alternative_3.png", device = "png",
       width = 1920, height = 1080, units = "px")
```
