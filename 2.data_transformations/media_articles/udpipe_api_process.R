...
conllu_to_df_mod <- function(x, document_id = "# newdoc", is_udpipe_annotation = FALSE, ...) {
  ...
  if ("term_id" %in% names(ldots)) {
    if (isTRUE(ldots$term_id)) {
      output_fields <- append(output_fields,
        values = "term_id",
        after = 4
      )
    }
  ...
  if ("start_end" %in% names(ldots)) {
    if (isTRUE(ldots$start_end)) {
      output_fields <- append(output_fields, values = c(
        "start",
        "end"
      ), after = 4)
    }
  ...
  default <- data.frame(
    doc_id = character(), paragraph_id = integer(),
  ...
  if (length(x$conllu) <= 1) {
    if (all(x$conllu == "")) {
      ...
      warning(sprintf(
        "No parsed data in x$conllu, returning default empty data.frame. Error message at x$error indicates e.g.: %s",
        msg
      ))
      return(default)
    }
  ...
  out <- data.table::data.table(txt = txt, doc_id = na_locf(ifelse(is_newdoc,
    sub("^# newdoc", document_id, txt), NA_character_
  )), sentence_id = na_locf(ifelse(is_sentenceid,
    sub("^# sent_id = *", "", txt), NA_character_
  )), sentence = na_locf(ifelse(is_sentencetext,
    sub("^# text = *", "", txt), NA_character_
  ...
  out <- out[, `:=`(c(
    ...
    "misc"
  ), data.table::tstrsplit(txt, "\t", fixed = TRUE))]
  ...
  if (all(c("start", "end") %in% output_fields)) {
    out[, `:=`(c("start", "end"), udpipe_reconstruct(
