...
print(f"""The following chunk(s) will will be processed: \n {chunks}""")
...
print("End in:", time() - start, "seconds.")
...
```{r}
list_of_processed_chunks <- list.files(path = "media_articles/data/regex_processed/add", pattern = "*.rds", full.names = TRUE)
...
# Get correct indexes for other datasets to compare
```{r}
list_of_processed_chunks <- list.files(path = "media_articles/data/regex_processed/chunks", pattern = "*.rds", full.names = TRUE)

list_of_indexes <- vector("list", length = length(list_of_processed_chunks)) %>% setNames(substr(list_of_processed_chunks, 64, nchar(list_of_processed_chunks) - 4))

for (i in seq_along(list_of_processed_chunks)) {
  list_of_indexes[[i]] <- list_of_processed_chunks[[i]] %>%
    readRDS() %>%
    select(article_id)
}

list_of_indexes <- list_of_indexes %>% bind_rows(.id = "file")

saveRDS(list_of_indexes, "correct_article_index.rds")
```
