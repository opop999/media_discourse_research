...
combined_counts %>% 
  ...
  geom_text(aes(x = key_dates[2], y = 0, label = names(key_dates)[2]), color = "#22209F", vjust = -27, size = 1.5, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[3], y = 0, label = names(key_dates)[3]), color = "#22209F", vjust = -23, size = 1.5, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[4], y = 0, label = names(key_dates)[4]), color = "#22209F", vjust = -18, size = 1.5, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[5], y = 0, label = names(key_dates)[5]), color = "#22209F", vjust = -24, size = 1.5, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[6], y = 0, label = names(key_dates)[6]), color = "#22209F", vjust = -30, size = 1.5, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[7], y = 0, label = names(key_dates)[7]), color = "#22209F", vjust = -21, size = 1.5, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[8], y = 0, label = names(key_dates)[8]), color = "#22209F", vjust = -15, size = 1.5, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[9], y = 0, label = names(key_dates)[9]), color = "#22209F", vjust = -54, size = 1.5, fontface = "bold", check_overlap = TRUE) +
...
combined_counts %>% 
  ...
  geom_vline(xintercept = c(as.numeric(key_dates[1:length(key_dates)])), color = "#563a3a", linetype = 2, size = 0.2) +
  geom_text(aes(x = key_dates[1], y = 0, label = names(key_dates)[1]), color = "#22209F", vjust = -22, size = 1.5, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[2], y = 0, label = names(key_dates)[2]), color = "#22209F", vjust = -37, size = 1.5, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[3], y = 0, label = names(key_dates)[3]), color = "#22209F", vjust = -29, size = 1.5, fontface = "bold", check_overlap = TRUE) +
...
```{r}
# GRAPH 3: Proportion of migration content thorough time, grouped by media type, 2015-2022
combined_counts_grouped %>% 
  ggplot(aes(x = month, y = migration_prop, color = media_type)) +
  geom_line() +
  geom_point(size = 0.5) +
  geom_vline(xintercept = c(as.numeric(key_dates[1:length(key_dates)])), color = "#563a3a", linetype = 2, size = 0.15) +
  geom_text(aes(x = key_dates[1], y = 0, label = names(key_dates)[1]), color = "#22209F", vjust = -22, size = 1.7, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[2], y = 0, label = names(key_dates)[2]), color = "#22209F", vjust = -37, size = 1.7, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[3], y = 0, label = names(key_dates)[3]), color = "#22209F", vjust = -29, size = 1.7, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[4], y = 0, label = names(key_dates)[4]), color = "#22209F", vjust = -25, size = 1.7, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[5], y = 0, label = names(key_dates)[5]), color = "#22209F", vjust = -24, size = 1.7, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[6], y = 0, label = names(key_dates)[6]), color = "#22209F", vjust = -30, size = 1.7, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[7], y = 0, label = names(key_dates)[7]), color = "#22209F", vjust = -21, size = 1.7, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[8], y = 0, label = names(key_dates)[8]), color = "#22209F", vjust = -15, size = 1.7, fontface = "bold", check_overlap = TRUE) +
  geom_text(aes(x = key_dates[9], y = 0, label = names(key_dates)[9]), color = "#22209F", vjust = -54, size = 1.7, fontface = "bold", check_overlap = TRUE) +
  ylab(element_blank()) +
  xlab(element_blank()) +
  labs(title = "Proportion of Migration Content in the Czech news media: 2015-2022",
       subtitle = "Percent of overall publishing output aggregated by month and split by media type",
       caption = "Data: Newton Media Archive, Media types: Vaclav Cvrcek & Jan Henys (2022), Search string: běženec* OR běženk* OR imigrant* OR migra* OR imigra* OR přistěhoval* OR uprchl* OR utečen* OR azylant*") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.text = element_text(size = 7),
        plot.background = element_rect(fill = "grey90"),
...
ggsave("../5.write_up/presentation_CCL_march_2022/graphics/migration_content_proportion_by_media_type.png", device = "png",
       width = 1920, height = 1080, units = "px")
...
combined_counts %>% 
  ggplot(aes(x = month, y = migration_prop)) +
    ...
    geom_text(aes(x = key_dates[1], y = 0, label = names(key_dates)[1]), vjust = -22, size = 1.7, check_overlap = TRUE) +
    geom_text(aes(x = key_dates[2], y = 0, label = names(key_dates)[2]), vjust = -40, size = 1.7, check_overlap = TRUE) +
    geom_text(aes(x = key_dates[3], y = 0, label = names(key_dates)[3]), vjust = -40, size = 1.7, check_overlap = TRUE) +
    geom_text(aes(x = key_dates[4], y = 0, label = names(key_dates)[4]), vjust = -46, size = 1.7, check_overlap = TRUE) +
    geom_text(aes(x = key_dates[5], y = 0, label = names(key_dates)[5]), vjust = -61, size = 1.7, check_overlap = TRUE) +
    geom_text(aes(x = key_dates[6], y = 0, label = names(key_dates)[6]), vjust = -46, size = 1.7, check_overlap = TRUE) +
    geom_text(aes(x = key_dates[7], y = 0, label = names(key_dates)[7]), vjust = -35, size = 1.7, check_overlap = TRUE) +
    geom_text(aes(x = key_dates[8], y = 0, label = names(key_dates)[8]), vjust = -22, size = 1.7, check_overlap = TRUE) +
    geom_text(aes(x = key_dates[9], y = 0, label = names(key_dates)[9]), vjust = -23, size = 1.7, check_overlap = TRUE) +
    ylab(element_blank()) +
    xlab(element_blank()) +
